...
\begin{equation}
\hat S^\dagger \hat  S=\mathbbm{1}\,, \qquad
S=1+i (2\pi)^d \delta^{(d)}(p_1+p_2-p_3-p_4) M(s,t,u)\,,
\label{eq:unitarity} 
\end{equation}
with $S=\braket{p_3,p_4 | \hat{S} | p_1, p_2}$ and the onshell momenta $p_i$ are expressed with Mandelstam invariants
\begin{equation}
s=(p_1+p_2)^2\,,\quad
t=(p_1-p_3)^2\,,\quad
u=(p_1-p_4)^2\,,\quad
...
\end{equation}
%
In the set-up below, we further assume \textit{maximal} analyticity, under its form of lightest particle maximal analtycity.
