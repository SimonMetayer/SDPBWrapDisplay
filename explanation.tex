...
The 2-to-2 scattering of identical massive ($m=1$) scalar particles is encoded by the S-matrix $S(s,t,u)$. Above threshold $s > 4$ and in any dimension $d\geq2+1$, it satisfies elastic \textbf{unitarity}
\begin{equation}
|S(s,t,u)|^2\leq1\,, 
\qquad
S(s,t,u)=1+i M(s,t,u)\,,
\label{eq:unitarity} 
\end{equation}
with Mandelstam invariants $s,t,u$. The amplitude $M$ satisfies \textbf{crossing} symmetry
\begin{equation}
...
\qquad
P_{d,\ell}(x)={}_2F_1\left(-\ell,d+\ell-3,\cfrac{d-2}{2},\cfrac{1-x}{2}\right)\,,
\end{equation}
...
\begin{equation}
f_\ell(s)=\cfrac{\mathcal{N}_d}{2}\int_{-1}^{1}\mathrm{d}x\,(1-x^2)^{\cfrac{d-4}{2}}\,P_{d,\ell}(x)\,M(s,t(s,x))\,, 
\qquad
x=\cos\theta=1+\cfrac{2t}{s-4}\,,
\end{equation}
...
\qquad
\phi^2(s)=\cfrac{(s-4)^{\cfrac{d-3}{2}}}{\sqrt{s}}\,.
\label{eq:unitarity2}
\end{equation}
Our normalization conventions read
\begin{equation}
n_{d,\ell}(x)=\cfrac{(4\pi)^{d/2}(d+2\ell-3)\Gamma(d+\ell-3)}{\pi\,\Gamma\left(\cfrac{d-2}{2}\right)\Gamma(\ell+1)}\,,
\qquad
\mathcal{N}_d=\cfrac{(16\pi)^{\cfrac{d-2}{2}}}{\Gamma\left(\cfrac{d-2}{2}\right)}\,.
\end{equation}
...
\begin{equation}
R_{\alpha\beta\gamma\delta}(s,t)=\cfrac{n_d}{6}\,\left(\rho^\gamma(s,\sigma_\alpha)\rho^\delta(t,\sigma_\beta)+ s \leftrightarrow t \leftrightarrow u \right)\,, 
\end{equation}
with $n_d=2^{5-d}n_{d,0}$ and the conformal map 
\begin{equation}
\rho(s,\sigma)=\cfrac{\sqrt{\sigma-4}-\sqrt{4-s}}{\sqrt{\sigma-4}+\sqrt{4-s}}\,,
\label{eq:confmap}
\end{equation}
...
\begin{equation}
\phi_n=\cfrac{\pi}{2}\left(1+\cos\cfrac{n\pi}{N+1}\right)\,, \qquad n=1,...,N\,, \qquad \phi_n\in (0,\pi)\,.
\end{equation}
...
\begin{equation}
\sigma_n=\left(\,\bigcup^\infty_{j=1}s_j\right)_{[1:n]}\,, \qquad n=1,...,N_\mathrm{max}\,, \qquad \sigma_n \in (4,\infty)\,.
\label{eq:sigmagrid}
\end{equation}
...
The numerics achieve perfection at $\ell_\mathrm{max}\rightarrow\infty$, $s_\mathrm{max}\rightarrow\infty$, $N_\mathrm{max}\rightarrow\infty$ and $\pi_{\mathrm{max}}\geq1$ (one could also choose instead $N_\mathrm{max}\geq1$ and $\pi_{\mathrm{max}}\rightarrow\infty$). For reasonable convergence, we typically use $\pi_\mathrm{max}=1$, $\ell_\mathrm{max}\approx14$ spins or more, $s_\mathrm{max}=300$ points, so $s$ range from $4+10^{-9}$ up to $10^9$ and $N_\mathrm{max}\approx10$ wavelet centers or more, yielding for example $\sigma_i=\{4.03, 4.06, 4.15, 4.46, 4.97, 6.67, 11.3, 19.5, 52.6, 243.1\}$, so we expect our amplitude to satisfy elastic unitary roughly up to $s\approx100$.



\section*{Optimization problem}

The low energy ''couplings'' $c_n$ are derivatives of the amplitudes taken at the crossing symetric point (cs:  $s=t=u=4/3$)
\begin{equation}
c_n=\cfrac{\mathcal{N}_d}{2}\times 
\begin{cases}
c(0,0) & n=0 \\
\cfrac{1}{2}c(n,0) & n \text{ even} \\
c(n-1,1) & n>1 \text{ odd}
\end{cases}\,\,\,,
\qquad
c(n_1,n_2)=\left.\cfrac{1}{n_1!n_2!}\partial_s^{n_1}\partial_t^{n_2}M(s,t)\right|_\mathrm{cs}\,,
\end{equation}
where, e.g., $\lambda=c_0$ is the non-pertubative quartic coupling. Unitary \eqref{eq:unitarity2} can be written with the matrix
\begin{equation}
\det U_\ell(s)\geq1\, \qquad
U_\ell(s)=\begin{pmatrix}
1-\cfrac{1}{2}\phi^2(s)\operatorname{Im} f_\ell(s) &
...
P_{d,\ell}\left(1+\cfrac{2t}{s-4}\right)
f_\ell(s)
\right)\geq0\,,
\end{equation}
...
\begin{equation}
\Delta(s)=n_d\times\left(\cfrac{4-s}{4}\right)^{-\cfrac{d-3}{2}}\times
\begin{cases}
\cfrac{(-1)^{\cfrac{d-3}{2}}\pi}{\log{\cfrac{4-s}{4}}+1} & d \text{ odd} \\
\sin\left(\pi\cfrac{d-3}{2}\right) & \text{else}
\end{cases}\,\,\,,
\end{equation}
