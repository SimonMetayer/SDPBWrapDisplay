...
\end{equation}
with $\alpha_{\alpha\beta\gamma\delta}$ our free parameters, the $\rho$-wavelet ansatz
\begin{equation}
...
The low energy ``couplings'' $c(n)$ are derivatives of the amplitudes taken at the crossing symetric point (cs:  $s=t=u=4/3$)
\begin{equation}
...
\end{equation}
where, e.g., $\lambda=c(0)$ is the non-pertubative quartic coupling. Unitary \eqref{eq:unitarity} can be written as a matrix
\begin{equation}
...
\end{pmatrix}
\end{equation}
and the game is then to optimize (maximize/minimize) a given $c(n)$ using the semi-definite positive condition
\begin{equation}
U_\ell(s)\succeq 0
\end{equation}
This gives us $c_{\mathrm{min/max}}(n)$ with typically $n=0,2,3$ and fixes all the corresponding $\alpha_{\alpha\beta\gamma\delta}$ parameters, hence the amplitude.
